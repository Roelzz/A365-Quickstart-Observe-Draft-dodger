...
`observability.py` wires up to **three concurrent exporters** per span, gated by independent env vars. They are additive — turning on the OTLP mirror does *not* disable A365 native; both fire on every turn.

| Env var | Exporter | Where to view spans |
|---|---|---|
| `ENABLE_A365_OBSERVABILITY_EXPORTER=true` (+ `ENABLE_A365_OBSERVABILITY=true`) | **A365 first-party telemetry** (`Agent365ExporterOptions`) — primary | https://admin.microsoft.com → Agents → &lt;your agent&gt; → **Activity** tab. Native A365 observability; goes through Microsoft's first-party pipeline. |
| `OTEL_EXPORTER_OTLP_ENDPOINT=http://localhost:4317` | **OTLP mirror** (`BatchSpanProcessor` + `OTLPSpanExporter`) — additive | Aspire Dashboard at http://localhost:18888 (or any OTel collector). Recommended local demo surface — beautiful UI, real-time, no licensing. |
| `OBSERVABILITY_DEBUG=true` | **Console mirror** (`SimpleSpanProcessor` + `ConsoleSpanExporter`) — additive | Agent stdout — span JSON pretty-prints after every turn. Diagnostic-only; noisy. |
