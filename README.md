...
├── manifest/                          # Generated by `a365 publish` — upload manifest.zip via admin.microsoft.com
│   ├── manifest.json
...
- (path-fix commit) — write `demo-tenant.config.json` to project root.
- (graph-perms-fix commit) — app reg script adds all 6 Graph permissions a365 CLI requires.
- (phase-2A-completion commit) — blueprint registered, manifest zipped for upload.
