...
# --- OTLP / Azure Monitor exporter setup (opt-in via env vars) ---
...
def create_and_run_host(agent_class: type[AgentInterface], *agent_args, **agent_kwargs):
    ...
    # Add OTLP / Azure Monitor exporters alongside A365 cloud exporter (opt-in)
    if _otlp_endpoint or _appinsights_conn:
        from opentelemetry import trace
        from opentelemetry.sdk.trace import TracerProvider
        from opentelemetry.sdk.trace.export import BatchSpanProcessor

        provider = trace.get_tracer_provider()
        # The A365 configure() sets up a TracerProvider; add extra processors to it
        if isinstance(provider, TracerProvider):
            if _otlp_endpoint:
                ...
                logger.info(f"[OTEL] OTLP trace exporter configured -> {_otlp_endpoint}")
            if _appinsights_conn:
                from azure.monitor.opentelemetry.exporter import AzureMonitorTraceExporter
                provider.add_span_processor(
                    BatchSpanProcessor(AzureMonitorTraceExporter(connection_string=_appinsights_conn))
                )
                logger.info("[OTEL] Azure Monitor trace exporter configured")
...
class GenericAgentHost:
    ...
    def __init__(self, agent_class: type[AgentInterface], *agent_args, **agent_kwargs):
        ...
        # Defaults to empty (no auth handler) - set AUTH_HANDLER_NAME=AGENTIC for production
    ...
    async def _setup_observability_token(self, context: TurnContext, tenant_id: str, agent_id: str):
        ...
        try:
            logger.info(f"🔐 Attempting token exchange for observability... (tenant_id={tenant_id}, agent_id={agent_id})")
            ...
            logger.info(f"✅ Token exchange successful (tenant_id={tenant_id}, agent_id={agent_id})")
    ...
    def start_server(self, auth_configuration: AgentAuthConfiguration | None = None):
        ...
        if auth_configuration:
            # Wrap JWT middleware to skip health endpoint
            @web_middleware
            async def jwt_middleware_with_health_bypass(request, handler):
                # Skip JWT for health endpoint
                if request.path == "/api/health":
                    return await handler(request)
                return await jwt_authorization_middleware(request, handler)
            middlewares.append(jwt_middleware_with_health_bypass)
        ...
        # Use 0.0.0.0 for container/cloud deployment, localhost for local dev
