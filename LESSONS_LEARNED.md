...
- **The audit pipeline lags real time by 30 min – 24 h.** A turn fired in the last few minutes will not be in the audit log even when everything is wired correctly — the OTLP ingest endpoint accepts the span synchronously (`HTTP 200 / rejectedSpans:0`), but the unified audit pipeline batches and persists asynchronously with the same SLAs as the rest of the Office 365 Management Activity API ("typically within one hour, up to 24 h"). `scripts/query-audit.sh` now prints `Newest row: <ISO-stamp>Z (<N>m ago)` after every search so you can tell at a glance how stale the freshest row is. **For real-time visibility during a demo, use the Aspire mirror (§19) — Purview Audit is the persistence/governance surface, not the realtime one.** Don't open a Microsoft support ticket for a "missing" turn until you have at least 30 min of headroom and have confirmed the agent's stdout shows the `HTTP 200 success on attempt 1. … "rejectedSpans":0` line for that turn (which proves it reached A365 ingest).
